...
The default 2D map uses Leaflet with detailed OpenStreetMap raster tiles. The optional 3D view uses MapLibre, USGS overview imagery, higher-detail USDA NAIP imagery at close zoom, and OpenFreeMap/OpenStreetMap labels. Raised buildings are hidden to keep routes unobstructed. If WebGL or GPU acceleration is unavailable, the app stays in 2D and explains why. No paid map API key is required.
